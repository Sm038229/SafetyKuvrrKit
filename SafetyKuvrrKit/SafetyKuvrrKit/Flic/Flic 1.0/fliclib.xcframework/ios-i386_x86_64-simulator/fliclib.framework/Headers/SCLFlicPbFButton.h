//
//  @file SCLFlicPbFButton.h
//  @framework fliclib
//
//  Created by Anton Meier on 2016-02-02.
 ...
 *
 *  @discussion Represents the diffrent connection states that a Flic/PbF can be in at any given time.
 *
 */
typedef NS_ENUM(NSInteger, SCLFlicButtonConnectionState)
{
    /**
     * The Flic/PbF is currently connected. This means that the iOS device has an active link with the Flic/PbF and that data can be sent in both
    ...
    /**
     * The Flic/PbF is currently connecting. This means that the iOS device is either in the process of establishing a link with Flic/PbF or
     * that it is waiting for Flic/PbF to come within proximity so that such process can be initiated.
     */
 ...
 *
 *  @discussion Represents the different modes that you can configure the button to operate in. These modes typically affect both click latency and expected battery life.
 *
 */
typedef NS_ENUM(NSInteger, SCLFlicButtonMode) {
    /**
     * Using this mode will give your button the lowest possible click latency (typicaly less than 30 ms). However, since this mode increases current consumption significantly, this mode will only work
     * while the application is in the foreground. As soon as the app leaves the foreground it will be set back to the regular latency setting. Please only use this if you have a
     * very good reason for it, such as if you are designing a foreground game that requires it.
     * Since this is designed for foreground use the button will only advertise, at most, one minute after connection is lost. After one minute you will need to push the button
     * again in order to activate the advertisement.
     */
    SCLFlicButtonModeLowLatency = 0,
    /**
     * This is the default mode of the button. It will give you a good compromise between click latency (less than 280 ms) and current consumption. This is intended to be used with use-cases where
     * the button should be connected at all time regardless if the app is in the foreground or background. Once a connection is lost, the button will continue to advertise indefinitely in order to re-establish
     * the connection as soon as possible when the button and the device comes into range with eachother.
     */
    SCLFlicButtonModeNormal,
    /**
     * This mode will have the same click latency as the Normal mode while connected. However, in order to preserve battery while the button is not used for extended periods of time, the button will
     * only advertise, at most, one hour after connection is lost. After one hour you will need to push the button again in order to activate the advertisement again.
     */
 ...
 *  @enum SCLFlicError
 *
 *  @discussion These enums represents the different error codes that can be sent on both the SCLFlicPbFButton and SCLFlicPbFManager classes. Not all errors
...
typedef NS_ENUM(NSInteger, SCLFlicError)
{
    ...
    /**
     * This code is send when a code within the Bluetooth's Security Manager Specification occurs.
     * In this case the SMP code will be included in the Localized Description of the error's userInfo of the format: "SMP code: %d"
     */
 ...
 *  @class SCLFlicButton
 *
 *  @discussion An instance of this class represents a physical Flic or PbF.
 *              All commands will be directly passed to the physical Flic/PbF and any events occurring on the Flic/PbF will be passed along
 ...
 *  @property delegate
 *
 *  @discussion The delegate object that will receive events related to this particular Flic/PbF.
 *
 */
 ...
 *
 *  @discussion The public key of the button. This in the key that is used to identify the Flic/PbF on our backend.
 *
 */
 ...
 *  @property macAddress
 *
 *  @discussion This property exposes the real mac addres of the Flic/PbF. The returned string is 17 character long on the format "xx:xx:xx:xx:xx:xx".
 *
 */
 ...
 *  @property name
 *
 *  @discussion The Bluetooth device name of the Flic/PbF button.
 *
 */
 ...
 *              only available on Flic buttons that have been grabbed from the Flic App. For PbF buttons this will always be be white.
 *
 */
 ...
 *              Please note that this is only available on Flic buttons that have been grabbed from the Flic App. For PbF buttons will only be an empty string.
 *
 */
 ...
 *
 */
@property(nonatomic, readwrite) BOOL lowLatency DEPRECATED_MSG_ATTRIBUTE("Use mode instead. See SCLFlicButtonModeLowLatency.");

/*!
 *  @property triggerBehavior
 *
 *  @discussion This property specifies how the Flic/PbF press events should be handled. You are allowed to change this property any time you want.
 ...
 *  @property pressCount
 *
 *  @discussion This property specifies how many times the Flic/PbF has been toggled at any given time. This will register all down events
 *              as well as the up events, which means that if you want to know how many times it has been clicked then you have to divide
 *              this number by two. Also, this property will always contain the last known registered value, meaning that if the Flic/PbF
 *              has been pressed while not being within proximity then the property will of course not be up to date. It will be updated
 *              as soon as the Flic/PbF connects the next time. Important notice: Even though this value is represented by an int the internal
 ...
 *              that anyone will ever reach those numbers. The presscounter will be reset if a factory reset is done on the physical Flic/PbF button.
 *
 */
 ...
 *  @property isReady
 *
 *  @discussion This property lets you know wether the Flic/PbF is ready or not. Only when it is ready will you be able to recieve or send data to it.
 *
 */
 ...
 *
 *  @discussion This property specifies the last known battery status of the Flic/PbF. If the status is Unknown then it may take up to 24h before a correct value is calculated.
 *
 */
 ...
 *
 *  @discussion This property lets you decide if you want the Flic/PbF button to automatically disconnect after a specified amount of time after each button press.
 *              Doing this can reduce the power consumption of the Flic/PbF button and thus extend the battery life, but it may also increase the click latency
 ...
 *  @property mode
 *
 *  @discussion The mode that the button operates in. Please see the SCLFlicButtonMode enum descriptions to learn more.
 *
 */
 ...
 *  @method connect:
 *
 *  @discussion		Attempts to connect the button. If the Flic/PbF is not available, due to either being out of range or not advertising, then
 ...
 *  @method disconnect:
 *
 *  @discussion		Disconnect a currently connected Flic/PbF or cancel a pending connection.
 *
 */
- (void)disconnect;
 ...
 *  @method readRSSI
 *
 *  @discussion         A call to this method will read the RSSI (received signal strength indication) of the Flic/PbF while it is currently connected.
 ...
 *
 *  @discussion         An integer value representing the last known firmware version of this Flic/PbF.
 *
 */
 ...
 *  @param queued       This lets you know if the event is a queued event that happened before the Flic/PbF connected or if it is a real time
 *                      event.
 ...
 *                      queued up on Flic/PbF while it was out of range. The units is in seconds and will be rounded to the nearest second.
 *
 *  @discussion         The Flic/PbF registered a button down event.
 *
 */
 ...
 *                      queued up on Flic/PbF while it was out of range. The units is in seconds and will be rounded to the nearest second.
 *
 *  @discussion         The Flic/PbF registered a button up event.
 *
 */
 ...
 *                      queued up on Flic/PbF while it was out of range. The units is in seconds and will be rounded to the nearest second.
 *
 *  @discussion         The Flic/PbF registered a button click event.
 ...
 *                      queued up on Flic/PbF while it was out of range. The units is in seconds and will be rounded to the nearest second.
 *
 *  @discussion         The Flic/PbF registered a button double click event.
 *                      The behavior of this event depends on what SCLFlicButtonTriggerBehavior the triggerBehavior property is set to.
 *
 */
 ...
 *                      queued up on Flic/PbF while it was out of range. The units is in seconds and will be rounded to the nearest second.
 *
 *  @discussion         The Flic/PbF registered a button hold event.
 *                      The behavior of this event depends on what SCLFlicButtonTriggerBehavior the triggerBehavior property is set to.
 *
 */
 ...
 *  @param button       The SCLFlicButton object that the event came from.
 *
 *  @discussion         This delegate method is called every time the Flic/PbF physically connected to the iOS device, regardless of the reason for it.
 *                      Keep in mind that you also have to wait for the <code>flicButtonIsReady:</code> before the Flic/PbF is ready for use.
 ...
 *  @param button       The SCLFlicButton object that the event came from.
 *
 *  @discussion         This delegate method is called every time the Flic/PbF has sucessfully connected and the autheticity has been verified.
 *                      You will not receive any press events from the Flic/PbF before this callback has been sent. Typically this event will be sent
 *                      immediately after the <code>flicButtonDidConnect:</code> event. The verification process will take longer time the very
 *                      first time that the PbF (only PbF) is connected to the particular iOS device since a round-trip to the backend server has to
 *                      be completed. This also means that an active internet connection is needed during the first connect of the PbF.
 *
 */
 ...
 *
 *  @discussion         This delegate method is called every time the Flic/PbF has disconnected, regardless of the reason for it.
 ...
 *
 *  @param button       The SCLFlicButton object that the event came from.
 *  @param error        The error that caused the Flic/PbF to fail to connect.
 *
 ...
 *                      made then the Flic/PbF will always cancel the pending connection, regardless of what mode the Flic/PbF happens to be in.
 *                      This means that if you get a <code>flicButton:didFailToConnectWithError:</code> event and the Flic/PbF is in Background mode then you
