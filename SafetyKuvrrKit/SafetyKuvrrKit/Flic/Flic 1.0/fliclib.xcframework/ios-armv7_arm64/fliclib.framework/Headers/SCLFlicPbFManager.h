 ...
 *                  the  Flic/PbF . You need to use this in order to scan for, and discover, new buttons. The object will
 ...
 *                  the associated Flic/PbF objects using the <code>knownButtons:</code> method.
 *
 */
 ...
 *  @property delegate
 *
 *  @discussion     The delegate object that will receive all the Flic/PbF related events. See the definition of the
 ...
 *                  will be allowed. This means that no communication with a  Flic/PbF  can be made.
 */
 ...
 *  @param background       Flag that specifies if want to configure the Flic manager to support background execution. If YES, then you also
 ...
 *
 *  @return         The number of PbFs discovered in total. This includes both public and private PbFs. A discovery callback will not be sent
 *                  if the PbF is private, but it might still be useful to know if there are any PbFs around, hence the return parameter.
 *
 */
- (NSUInteger)stopScan;
 ...
 *                  You will not be able to do any communication with a  Flic/PbF  until you call <code>enable</enable>.
 *
 */
- (void)disable;
 ...
 *
 *  @param scheme       This is the callback URL that you have registered in your Plist file and want the Flic app to return the grabbed Flic to.
 *                  Remember that this is only the scheme name that you have registered, and NOT a full URL.
 *
 */
...
/*!
 *  @method prepareBluetoothLogWithMessage:completion:
 *
 *  @param message  Lets you include additional information that will be written to the log meta data. This could for example be a comment from an end user.
 *  @param completion   Only after a successful completeon is it safe to read out the logging folder.
 *
 *  @discussion     Calling this method will write the meta data file to be included in the bluetooth log. Any old meta data file will be overwritten.
 *                  You should call this method just before you intend to upload a bluetooth log as this ensure that all the useful information is included.
 *
 */
- (void)prepareBluetoothLogWithMessage:(NSString * _Nullable)message completion:(void (^ _Nonnull)(NSURL * _Nullable preparedFolder, NSError *  _Nullable error))completion;
 ...
 *  @param button       The SCLFlicButton object that was generated for the newly found PbF.
 *  @param RSSI         The RSSI value of the newly found button at the time of discovery.
 *
 *  @discussion     This delegate method is called every time the a new PbF is discovered, the SCLFlicButton object
 ...
 *                  as a genuine PbF until after it has been properly connected. Remember that you will only be able to scan for
 *                  PbFs that are owned by your company.
 *
 */
